...
// Include after admin.js in every admin page.
// ============================================================
...
function getSidebarHTML(activePage) {
  ...
  const pending = Store.getAppointments().filter(a => a.status === "pendiente").length;

  const navLinks = items.map(item => {
    const isActive = window.location.pathname.endsWith(item.href) || window.location.href.includes(item.href);
    const badgeHTML = item.badge && pending > 0 ? `<span class="badge-count">${pending}</span>` : "";
  ...
  <aside class="sidebar" id="sidebar" aria-label="Navegación del panel">
    ...
    <div class="sidebar-footer">
      ...
      <a href="#" onclick="Admin.logout();return false;">
...
// Auto-inject sidebar if mount exists
document.addEventListener("DOMContentLoaded", () => {
  const mount = document.getElementById("sidebar-mount");
  if (mount) mount.outerHTML = getSidebarHTML();
});
